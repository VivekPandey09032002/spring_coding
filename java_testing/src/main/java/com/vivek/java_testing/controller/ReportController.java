...
import com.vivek.java_testing.dto.response.ResponseDailyReport;
import com.vivek.java_testing.dto.response.ResponseMonthlyReport;
...
public class ReportController {
    ...
    @GetMapping("/daily")
    public ResponseEntity<ResponseCollection<Object>> generateDailyReport() {
        modelMapper.getConfiguration().setMatchingStrategy(MatchingStrategies.LOOSE)
                .setSkipNullEnabled(true);
        final var userReports = attendanceRepository.findByAttendanceDate(LocalDate.now());
        log.debug("number of attendance found: {}", userReports.size());
        final var responseDto = userReports.stream()
                .map((userReport) -> modelMapper.map(userReport, ResponseDailyReport.class))
                .toList();
        for (ResponseDailyReport report : responseDto) {
            LocalTime currentTime = LocalTime.parse(loginTime, DateTimeFormatter.ISO_LOCAL_TIME);
            LocalTime inTime = report.getInTime();
        ...
        final var responseBody = ResponseCollection.builder().data(Collections.singletonList(responseDto))
    ...
    @GetMapping("/monthly")
    public ResponseEntity<ResponseCollection<Object>> generateMonthlyReport() {
        LocalTime currentTime = LocalTime.parse(loginTime, DateTimeFormatter.ISO_LOCAL_TIME);
        LocalDate firstMonthDate = LocalDate.now().withDayOfMonth(1);
        List<User> users = userRepository.findAll();
        log.debug("number of users found : {}", users.size());
        ...
        final var responseBody = ResponseCollection.builder().data(Collections.singletonList(reportList))
                .message("user saved successfully").status(HttpStatus.CREATED).build();
        log.info("monthly response generated ");
        return ResponseEntity.ok(responseBody);
